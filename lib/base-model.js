...
proto._populateCollection = function() {
  var self = this;
  var collection = self._data._collection;
  ...
  if(Array.isArray(collection)) {
    ...
    Object.keys(collection)
      .forEach(function(key) {
        ...
        outObj[key] = self._lance.wrap(item, self);
      });
    self._collection = outObj;
  }
}
...
proto.fetch = function(linkName, opts) {
  var self = this;
  opts = xtend({}, opts);
  var links = dotty.get(self._data, '_links');
  var uri = '';

  if(links) {
    links = Object.keys(links).reduce(function(obj, key) {
      var normalizedLinkName = links[key].href.split('?')[0];
      obj[normalizedLinkName] = links[key].href;
      return obj;
    }, {});
  }

  var uri = links[linkName.split('?')[0]];
  if(!uri) {
...
proto.isCollection = function() {
  var self = this;
  return (self.get('_collection')) ? true : false;
}

proto.hasClass = function() {
  var self = this;
  return (self.get('_meta.class')) ? true : false;
}
